...
@register("astrbot_plugin_context_compressor", "kitakita0421", "无感压缩上下文", "2.1.2")
class CanonicalCompressorPlugin(Star):
    ...
    async def compress_command(self, event: AstrMessageEvent):
        ...
        logger.info(f"[无感压缩上下文] 侦听到用户 {event.get_sender_id()} 手动输入 /compress 指令")
        ...
        if await self._run_canonical_compression(event, force_trigger=True):
            ...
            yield event.plain_result("⚠️ 压缩未触发。当前非系统消息原文轮数过少，未达到最低保留额度。")
        ...
        for i in range(25):
            ...
            try:
                ...
                if current_count > initial_count and current_history[-1].get("role") == "assistant":
                    logger.info(f"[无感压缩上下文] 🔔 [后台任务] AI回复已安全落库 (消息数: {initial_count} -> {current_count})。拉起自适应轮数检测...")
                    break
            except Exception:
                pass
                
            if i == 24:
                logger.warning("[无感压缩上下文] ⏳ [后台任务] 轮询达到安全时限，强行进入检测管道...")

        try:
            await self._run_canonical_compression(event, force_trigger=False)
        except Exception as run_err:
            logger.error(f"[无感压缩上下文] ❌ [后台任务] 执行核心自适应压缩时崩溃: {run_err}", exc_info=True)
    ...
    async def _run_canonical_compression(self, event: AstrMessageEvent, force_trigger: bool = False) -> bool:
        ...
        try:
            ...
            msg_obj_list = [Message(role=m.get("role"), content=m.get("content"), tool_calls=m.get("tool_calls")) for m in raw_history]
        except Exception:
            return False
        ...
        if hasattr(event, "raw_event") and isinstance(event.raw_event, dict):
            runtime_provider = event.raw_event.get("provider_id") or event.raw_event.get("chat_provider_id")
        ...
        logger.info(f"[无感压缩上下文] 📊 环境核对矩阵: Model='{runtime_provider}', BotInstance='{runtime_platform_id}', Type='{runtime_chat_type}'")
        ...
        logger.info(f"[无感压缩上下文] 轮数审计: 当前物理对话轮数 = {current_turns} 轮 (阈值限制 = {max_turns} 轮)")
        ...
        if not should_compress:
            if current_turns >= max_turns:
                ...
                logger.info(f"[无感压缩上下文] 🚦 满足水线: 物理轮数达到上限 ({current_turns} >= {max_turns} 轮)")
            elif max_tokens > 0:
                estimated_tokens = self.token_counter.count_tokens(non_system_objs)
                if estimated_tokens >= max_tokens:
                    should_compress = True
                    logger.info(f"[无感压缩上下文] 🚦 满足水线: 估算 Token 突破上限 ({estimated_tokens} >= {max_tokens})")
        ...
        llm_payload = messages_to_summarize + [Message(role="user", content=summary_instruction)]
        
        logger.info(f"[无感压缩上下文] 🚀 后台指派总结模型: '{final_summary_provider}' 归档前 {split_index // 2} 轮历史...")
        llm_resp = await self.context.llm_generate(chat_provider_id=final_summary_provider, prompt=None, contexts=llm_payload)
        if not llm_resp or not llm_resp.completion_text:
            logger.error(f"[无感压缩上下文] 调取模型 {final_summary_provider} 生成总结失败。")
            return False
        ...
        new_history = system_dicts + [
            {"role": "user", "content": f"Our previous history conversation summary: {summary_content}"},
            {"role": "assistant", "content": "Acknowledged the summary of our previous conversation history."}
        ] + recent_dicts

        await conv_mgr.update_conversation(unified_msg_origin=uid, conversation_id=curr_cid, history=new_history)
        logger.info(f"[无感压缩上下文] 🎉 🎉 🎉 成功！已通过模型 [{final_summary_provider}] 原地重构会话历史。")
        return True
